...
class UnmodSearchValidationWindow(wx.Frame):
    def __init__(
        ...
        n_mod_add, n_mod_sub, c_mod_add, c_mod_sub, rough_accuracy, fine_accuracy,
        auto_accuracy, min_score, ion_type
    ):
        ...
        self.calibration_info_file = os.path.join(
            self.directory_path,
            f"{self.basename}.calibrationInformation.txt"
        )
        self.peak_assignment_file = os.path.join(
            self.directory_path,
            f"{self.basename}.assignedPeakList.csv"
        )
        self.rough_accuracy = rough_accuracy
        self.fine_accuracy = fine_accuracy
        ...
        self.matched_theoretical_b_ions = []
        self.matched_theoretical_y_ions = []
        self.matched_observed_b_ions = []
        self.matched_observed_y_ions = []
        self.mean_error = 0.0
    ...
    def construct_matched_df(self):
        ...
        self.matched_theoretical_b_ions, self.matched_observed_b_ions = (
            self.report_matches_for_protein(theo_b_ions[0], self.peak_list, self.rough_accuracy)
        )
        self.matched_theoretical_y_ions, self.matched_observed_y_ions = (
            self.report_matches_for_protein(theo_y_ions[0], self.peak_list, self.rough_accuracy)
        )

        # calculate mean error of fits to help user filter
        error_list = []
        for i, j in zip(self.matched_theoretical_b_ions, self.matched_observed_b_ions):
            j = j[0]
            mass1 = theo_b_ions[0][i]
            mass2 = float(self.assignment_df["monoisotopic_mw"].iloc[j])
            error = -1 * (mass1 - mass2) / mass2 * 1000000
            error_list.append(error)
        for i, j in zip(self.matched_theoretical_y_ions, self.matched_observed_y_ions):
            j = j[0]
            mass1 = theo_y_ions[0][i]
            mass2 = float(self.assignment_df["monoisotopic_mw"].iloc[j])
            error = -1 * (mass1 - mass2) / mass2 * 1000000
            error_list.append(error)
        self.mean_error = statistics.mean(error_list)
        ...
        for i in assigned_peaks:
            ...
            self.matched_theo_loss_ions, self.matched_observed_loss_ions = (
                self.report_matches_for_protein(
                    theo_ions, self.peak_list, self.fine_accuracy*1.4142
                )# * sqrt(2) to account for two way comparison
            )
    ...
    def get_indices_below_n_percent(self, lst, n):
        ...
        # Calculate n% of the maximum value
        threshold = n / 100 * max_value

        # Use NumPy to get the indices of values below 10% of the maximum
    ...
    def calibrate_spectrum(self):
        ...
        ax1.scatter(x, y, color='black', marker='o', alpha=0.5)
        ax1.plot(x, regression_line, color='black')
        ax1.set_ylabel('Error (Th)')

        ax2.scatter(x, ppm_residuals, color='black', marker='o', alpha = 0.5)
    ...
    def filter_results(self):
        ...
        ppm_threshold = self.fine_accuracy
...
class ValidationPlotsPanel(wx.Panel):
    ...
    def plot_main_validation_spectrum(
        ...
        mean_error = peak_params[2]
        ion_name = peak_params[3]
        adduct = peak_params[4]
        loss = peak_params[5]
        ...
        if scatter_x is not None:
            ...
            label_text = (
                ...
                f"Error from mean: {round(ppm_error - mean_error, 2)} ppm"
            )
