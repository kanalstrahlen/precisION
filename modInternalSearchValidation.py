...
class ModInternalSearchValidationWindow(wx.Frame):
    def __init__(
        ...
        fine_accuracy, auto_accuracy, min_score, e_value, mod_details#, ion_type
    ):
    ...
    def identify_sets(self, expected):
        ...
        for j in range(len(self.sequence)):
            ...
            theo_b_ions = [theo_ion_mass + self.mod_mass_shift for theo_ion_mass in theo_b_ions]
        ...
        for j in range(len(self.sequence)):
            ...
            theo_y_ions = [ion_mass + self.mod_mass_shift - 18.0105646863 for ion_mass in theo_y_ions]
    ...
    def gen_internal_fragments(self, sequence, start_index, n_term=True):
        ...
        if n_term:
            ion_masses = np.cumsum(aa_masses) + self.mod_mass_shift
            ion_masses = ion_masses[:-1]
        else:
            ion_masses = sequence_mass - np.cumsum(aa_masses) - 18.0105646863 + self.mod_mass_shift
            ion_masses = ion_masses[:-1]
            ion_masses.sort()
